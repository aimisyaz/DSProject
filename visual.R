...
ggplot(summary_long, aes(x = age_group, y = Value, fill = Metric)) +
  ...
  labs(title = "Mean vs. Median Rating by Age Group",
       ...
       x = "Age Group",
       y = "Rating Value") +
  coord_cartesian(ylim = c(1, 5)) + # Focus on the 1-5 star range
  theme_minimal()


# Calculate group centers for the plot
...
ggplot(data, aes(x = age, y = rating, color = age_group)) +
  # 1. Faded jittered points for the background "noise"
  geom_jitter(alpha = 0.2, width = 0.5, height = 0.2) +
  
  # 2. Large, solid points for the Group Means
  geom_point(data = group_means, 
             aes(x = mean_age, y = mean_rating), 
             color = "black", size = 5, shape = 18) +
  
  # 3. Add a trend line per group to see if older/younger people within the class differ
